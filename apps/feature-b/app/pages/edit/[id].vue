<template>
  <div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-4">プロフィール編集</h1>
      ...
      <!-- プロフィールが見つからない場合 -->
      <div v-if="!profile" class="bg-yellow-50 border border-yellow-200 rounded-md p-4">
        <div class="flex">
      ...
      <form v-else @submit.prevent="handleSubmit" class="bg-white shadow-lg rounded-lg p-6">
        ...
        <div class="mt-8 bg-gray-50 p-4 rounded-md">
          ...
          <div class="grid grid-cols-2 gap-4 text-sm text-gray-600">
            <div>
              <span class="font-medium">作成者:</span> {{ profile.createdBy }}
            </div>
            <div>
              <span class="font-medium">作成日:</span> {{ formatDateTime(profile.createdAt) }}
            </div>
            <div>
              <span class="font-medium">更新者:</span> {{ profile.updatedBy }}
            </div>
            <div>
              <span class="font-medium">更新日:</span> {{ formatDateTime(profile.updatedAt) }}
            </div>
          </div>
        </div>
...
import { formatDateTime } from '../../utils/profile'

const route = useRoute()
const router = useRouter()
const { getProfileById, updateProfile, deleteProfile } = useProfileStore()
...
const handleSubmit = async () => {
  if (isSubmitting.value || !profile) return
  ...
  try {
    ...
    // プロフィールを更新
    updateProfile(profileId, form.value, 'システムユーザー')
...
const handleDelete = async () => {
  ...
  try {
    isSubmitting.value = true
    deleteProfile(profileId, 'システムユーザー')
    await router.push('/')
  } catch (error) {
    errorMessage.value = 'エラーが発生しました。もう一度お試しください。'
...
if (!profile) {
  ...
  useSeoMeta({
    title: `編集: ${profile.lastName} ${profile.firstName}`
  })
}
</script>
